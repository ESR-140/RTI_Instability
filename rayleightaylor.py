...
def getConserved( rho, vx, vy, P, gamma, vol ):       #Calculate the conserved variable from the primitive
	Mass   = rho * vol
	Momx   = rho * vx * vol
	Momy   = rho * vy * vol
...
def getPrimitive( Mass, Momx, Momy, Energy, gamma, vol ):   #Calculate the primitive variable from the conservative
	rho = Mass / vol
	vx  = Momx / rho / vol
	vy  = Momy / rho / vol
...
def getFlux(rho_L, rho_R, vx_L, vx_R, vy_L, vy_R, P_L, P_R, gamma): #Calculate fluxed between 2 states with local Lax-Friedrichs/Rusanov rule 
...
def addSourceTerm( Mass, Momx, Momy, Energy, g, dt ): #Add gravitational source term to conservative variables
